...
def get_domain_security_config(admin_id: str) -> Dict[str, Any]:
    """
    주어진 관리자 ID에 연결된 도메인 및 보안 정보를 DB에서 조회합니다.
    """
    conn = get_db_connection()
    cursor = conn.cursor()
    
    cursor.execute("SELECT domain_name, ssl_status FROM domain_security WHERE admin_id = ?", (admin_id,))
    result = cursor.fetchone()
    conn.close()
...
def update_domain_security_config(admin_id: str, domain_name: str, ssl_status: str) -> bool:
    """
    주어진 관리자 ID의 도메인 및 보안 정보를 DB에 저장하거나 업데이트합니다 (UPSERT).
    """
    conn = get_db_connection()
    cursor = conn.cursor()
    ...
    try:
        cursor.execute(
            """
            INSERT INTO domain_security (admin_id, domain_name, ssl_status)
            VALUES (?, ?, ?)
            ON CONFLICT(admin_id) DO UPDATE SET
                domain_name = excluded.domain_name,
                ssl_status = excluded.ssl_status
            """,
            (admin_id, domain_name, ssl_status)
        )
        conn.commit()
        return True
    except sqlite3.Error as e:
