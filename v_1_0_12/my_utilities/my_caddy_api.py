"""
Caddy Admin API를 통한 도메인 및 보안 관리 유틸리티

이 모듈은 Caddy의 Admin API를 사용하여 도메인 등록/해제 및
SSL/TLS 인증서 상태를 관리합니다.

환경 변수:
    CADDY_MOCK_MODE: "true"로 설정하면 모의(mock) 테스트 모드로 동작
                      실제 Caddy API를 호출하지 않고 가상 응답을 생성합니다.
                      Windows 로컬 테스트에 유용합니다.

인증서 재발급 로직:
    1. 로컬 캐시 확인: Caddy 데이터 디렉토리에 인증서가 있으면 재사용
    2. 자동 재발급: 로컬 캐시가 없어도 Caddy가 Let's Encrypt에서 자동 재발급 시도
    3. 중복 인증서 정책: 같은 도메인에 대해 주당 5개까지 중복 발급 가능
       - 재설치 후에도 도메인 소유권만 유효하면 새 인증서 발급 가능
       - Rate Limit이 일반 발급(주당 50개)보다 완화됨
    4. 도메인 소유권 검증: HTTP-01 또는 TLS-ALPN-01 챌린지를 통해 자동 검증
       - Caddy가 80번 또는 443번 포트에서 자동으로 챌린지 응답
       - DNS 설정만 올바르면 수동 작업 불필요

참고: Let's Encrypt는 보안상 이유로 발급된 인증서를 조회하는 API를 제공하지 않습니다.
      따라서 재설치 시 Caddy 데이터가 완전히 삭제되면, 도메인 소유권 재검증을 통해
      새 인증서를 자동으로 발급받는 방식을 사용합니다.
"""

import requests
import time
import json
import os
from typing import Tuple, Dict, Optional, Generator

# 모의(Mock) 테스트 모드 확인
# Windows 로컬 테스트: set CADDY_MOCK_MODE=true
# Vultr 프로덕션: 환경 변수 설정 안 함 (기본값 false)
MOCK_MODE = os.environ.get("CADDY_MOCK_MODE", "false").lower() == "true"

# Caddy Admin API 기본 URL
CADDY_API_URL = "http://127.0.0.1:2019"

# 고정 IP 주소 (내집 IP)
MY_IP = "61.85.61.62"

# 모의 모드 알림
if MOCK_MODE:
    print("=" * 60)
    print("🎭 [CADDY MOCK MODE 활성화]")
    print("   실제 Caddy API를 호출하지 않습니다.")
    print("   Windows 로컬 테스트 모드로 동작합니다.")
    print("=" * 60)


def get_current_config() -> Optional[Dict]:
    """
    현재 Caddy 설정을 가져옵니다.

    Returns:
        현재 설정 딕셔너리 또는 None (실패 시)
    """
    try:
        response = requests.get(f"{CADDY_API_URL}/config/")
        if response.status_code == 200:
            return response.json()
        else:
            print(f">> Caddy 설정 가져오기 실패: {response.status_code}")
            return None
    except Exception as e:
        print(f">> Caddy API 연결 실패: {e}")
        return None


def check_cert_status(domain: str) -> Tuple[str, str]:
    """
    도메인의 SSL/TLS 인증서 발급 상태를 확인합니다.

    Args:
        domain: 확인할 도메인

    Returns:
        (상태, 메시지) 튜플
        상태: "pending", "active", "failed", "unknown"
    """
    try:
        response = requests.get(f"{CADDY_API_URL}/config/apps/tls/certificates")
        if response.status_code == 200:
            certs = response.json()

            # 인증서 목록에서 도메인 찾기
            for cert_info in certs:
                if isinstance(cert_info, dict) and 'subjects' in cert_info:
                    if domain in cert_info.get('subjects', []):
                        return "active", f"✅ {domain}에 대한 SSL/TLS 인증서가 활성화되었습니다."

            return "pending", f"⏳ {domain}에 대한 인증서 발급이 진행 중입니다..."
        else:
            return "unknown", "인증서 상태를 확인할 수 없습니다."
    except Exception as e:
        return "unknown", f"인증서 상태 확인 중 오류 발생: {e}"


def check_existing_certificate(domain: str) -> Tuple[bool, str]:
    """
    Caddy에 저장된 기존 인증서가 있는지 확인합니다.

    Args:
        domain: 확인할 도메인

    Returns:
        (기존 인증서 존재 여부, 상태 메시지)
        - True, "로컬 캐시": Caddy 로컬에 인증서 존재
        - False, "재발급 필요": 인증서 없음, 새로 발급 필요
        - False, "재발급 시도": 인증서 없지만 Let's Encrypt에서 자동 재발급 시도
    """
    try:
        # 1. Caddy Admin API로 현재 로드된 인증서 확인
        response = requests.get(f"{CADDY_API_URL}/config/apps/tls/certificates")
        if response.status_code == 200:
            certs = response.json()

            # 인증서 목록에서 도메인 확인
            for cert_info in certs:
                if isinstance(cert_info, dict) and 'subjects' in cert_info:
                    if domain in cert_info.get('subjects', []):
                        print(f"[Caddy API] ✅ 로컬 캐시에서 기존 인증서 발견: {domain}")
                        return True, "로컬 캐시"

        print(f"[Caddy API] ℹ️ 로컬 캐시에 인증서 없음: {domain}")
        print(f"[Caddy API] 💡 Caddy가 Let's Encrypt에서 자동으로 인증서를 재발급 시도합니다.")
        print(f"[Caddy API] 💡 도메인 소유권이 유효하면 중복 인증서 정책으로 빠르게 발급됩니다.")

        # Caddy 데이터가 없어도 도메인 소유권이 있으면 자동 재발급
        return False, "재발급 시도"

    except Exception as e:
        print(f"[Caddy API] ⚠️ 인증서 확인 중 오류: {e}")
        return False, "재발급 필요"


def check_rate_limit_error(response_text: str) -> Tuple[bool, Optional[Dict]]:
    """
    Caddy API 응답에서 Let's Encrypt Rate Limit 에러를 확인합니다.

    Args:
        response_text: Caddy API 응답 텍스트

    Returns:
        (Rate Limit 에러 여부, 에러 정보 딕셔너리)
        에러 정보: {"message": "에러 메시지", "retry_after": "재시도 가능 일시"}
    """
    try:
        # Let's Encrypt Rate Limit 키워드 확인
        rate_limit_keywords = [
            "rateLimited",
            "too many certificates",
            "rate limit",
            "urn:ietf:params:acme:error:rateLimited"
        ]

        response_lower = response_text.lower()
        is_rate_limited = any(keyword.lower() in response_lower for keyword in rate_limit_keywords)

        if is_rate_limited:
            # 기본 에러 정보
            error_info = {
                "message": "Let's Encrypt에서 인증서 발급 횟수 제한이 적용되었습니다.",
                "retry_after": None
            }

            # retry-after 정보 추출 시도 (간단한 파싱)
            if "retry after" in response_lower:
                # 추가 파싱 로직 (필요시)
                pass

            return True, error_info

        return False, None

    except Exception as e:
        print(f"[Caddy API] ⚠️ Rate Limit 확인 중 오류: {e}")
        return False, None


def register_domain_with_progress(domain: str, email: str = "") -> Generator[Dict[str, str], None, None]:
    """
    도메인을 등록하고 진행 상황을 실시간으로 yield합니다. (SSE용)

    환경 변수 CADDY_MOCK_MODE=true로 설정하면 모의 테스트 모드로 동작합니다.

    Args:
        domain: 등록할 도메인
        email: Let's Encrypt 알림용 이메일 (선택 사항, 생략 시 Let's Encrypt 기본 동작)

    Yields:
        {"status": "progress/success/error", "message": "메시지"} 형식의 딕셔너리
    """
    # 모의 모드일 경우 mock 함수 사용
    if MOCK_MODE:
        from my_utilities.my_caddy_api_mock import register_domain_with_progress_mock
        yield from register_domain_with_progress_mock(domain, email)
        return

    email_info = f", 이메일: {email}" if email else " (이메일 생략)"
    print(f"[Caddy API] 🚀 도메인 등록 함수 시작: {domain}{email_info}")
    try:
        # 0단계: 기존 인증서 확인
        print(f"[Caddy API] 🔍 기존 인증서 확인 중...")
        yield {
            "status": "progress",
            "message": "🔍 기존 인증서 확인 중...",
            "step": "0/5"
        }

        has_existing_cert, cert_status = check_existing_certificate(domain)
        if has_existing_cert:
            print(f"[Caddy API] ✅ 기존 인증서 발견! 재사용합니다.")
            yield {
                "status": "progress",
                "message": f"✅ {domain}의 로컬 캐시에서 기존 인증서를 발견했습니다. 재사용합니다.",
                "step": "0/5"
            }
            time.sleep(1)
        else:
            # 로컬 캐시는 없지만 Caddy가 자동으로 재발급 시도
            print(f"[Caddy API] ℹ️ 로컬 캐시에 인증서 없음. Caddy가 자동 재발급을 시도합니다.")
            yield {
                "status": "progress",
                "message": "ℹ️ 로컬 캐시에 인증서 없음. Caddy가 Let's Encrypt에서 자동으로 재발급을 시도합니다.\n💡 도메인 소유권이 유효하면 중복 인증서 정책으로 빠르게 발급됩니다.",
                "step": "0/5"
            }
            time.sleep(1)

        # 1단계: Caddyfile 업데이트 시작
        print(f"[Caddy API] 📋 1단계: Caddy 설정 생성 중...")
        yield {
            "status": "progress",
            "message": "⏳ Caddy 설정 업데이트 중...",
            "step": "1/5"
        }

        # Caddy 설정 생성 (도메인 + IP 제한)
        config = {
            "apps": {
                "http": {
                    "servers": {
                        "srv0": {
                            "listen": [":80", ":443"],
                            "routes": [
                                # 도메인 라우트 (HTTPS 자동 인증)
                                {
                                    "match": [{"host": [domain]}],
                                    "handle": [{
                                        "handler": "reverse_proxy",
                                        "upstreams": [{"dial": "127.0.0.1:8000"}]
                                    }],
                                    "terminal": True
                                },
                                # IP 제한 라우트 (HTTP)
                                {
                                    "@id": "ip_matcher",
                                    "match": [{
                                        "remote_ip": {
                                            "ranges": [f"{MY_IP}/32"]
                                        }
                                    }],
                                    "handle": [{
                                        "handler": "reverse_proxy",
                                        "upstreams": [{"dial": "127.0.0.1:8000"}]
                                    }],
                                    "terminal": True
                                },
                                # 기타 모든 요청 거부
                                {
                                    "handle": [{
                                        "handler": "static_response",
                                        "status_code": 403,
                                        "body": "Access Denied"
                                    }]
                                }
                            ]
                        }
                    }
                },
                "tls": {
                    "automation": {
                        "policies": [
                            {
                                "subjects": [domain],
                                "issuers": [
                                    {
                                        "module": "acme"
                                        # email은 선택 사항: 제공되면 추가, 없으면 생략
                                    }
                                ]
                            }
                        ]
                    }
                }
            }
        }

        # 이메일이 제공된 경우에만 추가
        if email:
            config["apps"]["tls"]["automation"]["policies"][0]["issuers"][0]["email"] = email

        time.sleep(0.5)

        # 2단계: Admin API로 설정 적용
        print(f"[Caddy API] 📋 2단계: Caddy Admin API로 설정 전송 중... (URL: {CADDY_API_URL}/load)")
        yield {
            "status": "progress",
            "message": "⏳ Caddy에 새 설정 적용 중...",
            "step": "2/5"
        }

        response = requests.post(
            f"{CADDY_API_URL}/load",
            json=config,
            headers={"Content-Type": "application/json"}
        )

        print(f"[Caddy API] 📡 Caddy 응답 코드: {response.status_code}")
        if response.status_code not in [200, 204]:
            # Rate Limit 에러 확인
            is_rate_limited, rate_limit_info = check_rate_limit_error(response.text)

            if is_rate_limited:
                error_msg = f"⚠️ {rate_limit_info['message']}\n\n기존 인증서가 있다면 재사용을 시도합니다. 없다면 나중에 다시 시도해주세요."
                print(f"[Caddy API] {error_msg}")
                yield {
                    "status": "warning",
                    "message": error_msg,
                    "rate_limit_info": rate_limit_info
                }
                # Rate Limit 에러는 경고로 처리하고 계속 진행
            else:
                error_msg = f"❌ Caddy 설정 적용 실패: {response.text}"
                print(f"[Caddy API] {error_msg}")
                yield {
                    "status": "error",
                    "message": error_msg
                }
                return

        print(f"[Caddy API] ✅ Caddy 설정 적용 성공")

        time.sleep(1)

        # 3단계: SSL/TLS 인증서 발급 요청 확인
        yield {
            "status": "progress",
            "message": f"⏳ {domain}에 대한 SSL/TLS 인증서 발급 요청 중...",
            "step": "3/5"
        }

        time.sleep(2)

        # 4단계: Let's Encrypt 인증서 검증 중
        # 기존 인증서가 있으면 대기 시간 단축
        if has_existing_cert:
            yield {
                "status": "progress",
                "message": "⏳ 기존 인증서 적용 확인 중 (최대 3초 소요)...",
                "step": "4/5"
            }
            max_wait_time = 3
        else:
            yield {
                "status": "progress",
                "message": "⏳ Let's Encrypt 인증서 검증 중 (최대 10초 소요)...",
                "step": "4/5"
            }
            max_wait_time = 10

        # 인증서 발급 완료 대기
        check_interval = 2
        elapsed_time = 0

        cert_active = False
        last_error_message = ""
        while elapsed_time < max_wait_time:
            time.sleep(check_interval)
            elapsed_time += check_interval

            cert_status, cert_message = check_cert_status(domain)

            if cert_status == "active":
                cert_active = True
                break
            elif cert_status == "failed":
                last_error_message = cert_message
                # 실패해도 계속 진행 (Caddy가 백그라운드에서 재시도할 수 있음)
                break

            # 진행 중 메시지 업데이트
            yield {
                "status": "progress",
                "message": f"⏳ 인증서 검증 중... ({elapsed_time}/{max_wait_time}초)",
                "step": "4/5"
            }

        # 5단계: 완료
        if cert_active:
            print(f"[Caddy API] ✅ 도메인 등록 완료: {domain} (인증서 활성화)")

            if has_existing_cert:
                cert_source = "로컬 캐시에서 기존 인증서 재사용"
            else:
                cert_source = "Let's Encrypt에서 자동 재발급"

            yield {
                "status": "success",
                "message": f"✅ HTTPS 인증 완료! {domain}으로 안전하게 접속할 수 있습니다.\n({cert_source})",
                "step": "5/5",
                "domain_name": domain,
                "security_status": "HTTPS"
            }
        else:
            # 인증서가 아직 발급 중이지만 설정은 완료됨
            print(f"[Caddy API] ✅ 도메인 등록 완료: {domain} (인증서 백그라운드 발급 중)")

            # 기존 인증서가 로컬 캐시에 있었는데 활성화 안 된 경우
            if has_existing_cert:
                yield {
                    "status": "success",
                    "message": f"✅ 도메인 설정 완료! 로컬 캐시의 기존 인증서를 적용했습니다.\n잠시 후 HTTPS로 접속해보세요.",
                    "step": "5/5",
                    "domain_name": domain,
                    "security_status": "HTTPS"
                }
            else:
                # Caddy가 자동으로 재발급 시도 중
                yield {
                    "status": "success",
                    "message": f"✅ 도메인 설정 완료! Caddy가 Let's Encrypt에서 자동으로 인증서를 재발급합니다.\n💡 도메인 소유권이 유효하면 중복 인증서 정책으로 빠르게 발급됩니다. (최대 1분 소요)",
                    "step": "5/5",
                    "domain_name": domain,
                    "security_status": "HTTPS"
                }

    except Exception as e:
        error_msg = f"❌ 오류 발생: {str(e)}"
        print(f"[Caddy API] {error_msg}")
        yield {
            "status": "error",
            "message": error_msg
        }


def release_domain_with_progress(domain: str) -> Generator[Dict[str, str], None, None]:
    """
    도메인을 해제하고 HTTP로 되돌리며, 진행 상황을 실시간으로 yield합니다. (SSE용)

    환경 변수 CADDY_MOCK_MODE=true로 설정하면 모의 테스트 모드로 동작합니다.

    Args:
        domain: 해제할 도메인 (표시 용도)

    Yields:
        {"status": "progress/success/error", "message": "메시지"} 형식의 딕셔너리
    """
    # 모의 모드일 경우 mock 함수 사용
    if MOCK_MODE:
        from my_utilities.my_caddy_api_mock import release_domain_with_progress_mock
        yield from release_domain_with_progress_mock(domain)
        return

    try:
        # 1단계: 도메인 설정 제거 시작
        yield {
            "status": "progress",
            "message": "⏳ 도메인 설정 제거 중...",
            "step": "1/3"
        }

        # IP만 허용하는 설정 (HTTP만)
        config = {
            "apps": {
                "http": {
                    "servers": {
                        "srv0": {
                            "listen": [":80"],
                            "routes": [
                                # IP 제한 라우트만 유지
                                {
                                    "match": [{
                                        "remote_ip": {
                                            "ranges": [f"{MY_IP}/32"]
                                        }
                                    }],
                                    "handle": [{
                                        "handler": "reverse_proxy",
                                        "upstreams": [{"dial": "127.0.0.1:8000"}]
                                    }],
                                    "terminal": True
                                },
                                # 기타 모든 요청 거부
                                {
                                    "handle": [{
                                        "handler": "static_response",
                                        "status_code": 403,
                                        "body": "Access Denied"
                                    }]
                                }
                            ]
                        }
                    }
                }
            }
        }

        time.sleep(0.5)

        # 2단계: Admin API로 설정 적용
        yield {
            "status": "progress",
            "message": "⏳ HTTP 전용 설정 적용 중...",
            "step": "2/3"
        }

        response = requests.post(
            f"{CADDY_API_URL}/load",
            json=config,
            headers={"Content-Type": "application/json"}
        )

        if response.status_code not in [200, 204]:
            yield {
                "status": "error",
                "message": f"❌ Caddy 설정 적용 실패: {response.text}"
            }
            return

        time.sleep(1)

        # 3단계: 완료
        yield {
            "status": "success",
            "message": f"✅ 도메인 ({domain}) 해제 완료! HTTP로 되돌려졌습니다.",
            "step": "3/3",
            "domain_name": "없음",
            "security_status": "HTTP"
        }

    except Exception as e:
        yield {
            "status": "error",
            "message": f"❌ 오류 발생: {str(e)}"
        }


def register_domain(domain: str, email: str = "admin@hanane.kr") -> Tuple[bool, str]:
    """
    도메인을 등록합니다. (비-SSE 버전, 백업용)

    Args:
        domain: 등록할 도메인
        email: Let's Encrypt 알림용 이메일 (기본값: admin@hanane.kr)

    Returns:
        (성공 여부, 메시지)
    """
    try:
        for progress in register_domain_with_progress(domain, email):
            if progress["status"] == "error":
                return False, progress["message"]
            elif progress["status"] == "success":
                return True, progress["message"]

        return False, "알 수 없는 오류 발생"
    except Exception as e:
        return False, f"도메인 등록 실패: {e}"


def release_domain(domain: str) -> Tuple[bool, str]:
    """
    도메인을 해제합니다. (비-SSE 버전, 백업용)

    Args:
        domain: 해제할 도메인

    Returns:
        (성공 여부, 메시지)
    """
    try:
        for progress in release_domain_with_progress(domain):
            if progress["status"] == "error":
                return False, progress["message"]
            elif progress["status"] == "success":
                return True, progress["message"]

        return False, "알 수 없는 오류 발생"
    except Exception as e:
        return False, f"도메인 해제 실패: {e}"
