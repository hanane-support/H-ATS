# Caddy Admin API를 사용한 도메인 관리 라우터
...
async def domain_manager(request: Request):
    ...
    if not admin_id:
        ...
        db_domain = domain_config.get("domain_name", "없음")
        db_security = domain_config.get("security_status", "HTTP")

        print(f"[Domain Manager] 📊 DB 상태: domain={db_domain}, security={db_security}")

        # ========================================
        # 🔄 2단계: Caddy Admin API에서 도메인 조회
        # ========================================
        caddy_domain = get_domain_from_caddy()

        # ========================================
        # 🔄 3단계: DB와 Caddy 동기화
        # ========================================

        # 케이스 1: Caddy에 도메인이 있지만 DB에 없음 (또는 다름)
        # → Caddy 우선: Caddy 설정을 DB에 저장
        if caddy_domain and (db_domain == "없음" or db_domain != caddy_domain):
            print(f"[Domain Manager] 🔄 Caddy → DB 동기화 시작: {caddy_domain}")

            # HTTPS 활성화 여부 확인
            is_https = check_if_https_active(caddy_domain)
            new_security_status = 'HTTPS' if is_https else 'HTTP'

            # DB 업데이트
            update_success = update_domain_security_config(
                admin_id,
                caddy_domain,
                new_security_status,
                ""  # 이메일은 빈 문자열
            ...
            if update_success:
                print(f"[Domain Manager] ✅ DB 동기화 성공: {caddy_domain} ({new_security_status})")
                # 업데이트된 정보를 다시 조회
                domain_config = get_domain_security_config(admin_id)
            else:
                print(f"[Domain Manager] ❌ DB 동기화 실패")

        # 케이스 2: DB에 도메인이 있지만 Caddy에 없음
        # → 경고: Caddy 설정이 손실되었음을 로그에 표시
        elif not caddy_domain and db_domain != "없음":
            print(f"[Domain Manager] ⚠️ 경고: DB에 {db_domain} 있으나 Caddy에 없음 (재등록 필요)")
            # DB 정보를 유지하되, 사용자에게 재등록을 권장할 수 있음

        # 케이스 3: 둘 다 같음 또는 둘 다 없음
        # → 정상 상태, 동기화 불필요
        else:
            print(f"[Domain Manager] ✅ DB와 Caddy 동기화됨")
...
@domain_router.post("/domain/apply_security")
async def apply_security(request: Request):
    ...
    async def event_stream():
        ...
        print(f"🚀 도메인 등록 시작: {domain_to_register} (admin_id: {admin_id})")
        for progress in register_domain_with_progress(domain_to_register, "", admin_id):
            ...
            if progress["status"] == "success":
                ...
                print(f"⚠️ 도메인 설정 완료, 인증서 발급 실패: {progress.get('message')}")
                # warning 상태에서는 DB 업데이트 하지 않음 (HTTP 상태 유지)
                break
...
@domain_router.post("/domain/release_security")
async def release_security(request: Request):
    ...
    print(f"✅ 도메인 해제 요청 받음 (관리자 ID: {admin_id})")

    # SSE 스트림 생성
    async def event_stream():
        """도메인 해제 진행 상황을 SSE로 스트리밍"""
        print(f"🚀 도메인 해제 시작 (admin_id: {admin_id})")
        for progress in release_domain_with_progress(admin_id):
            print(f"📡 SSE 전송: {progress}")
            yield sse_event(progress)
